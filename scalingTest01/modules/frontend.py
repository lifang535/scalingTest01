    ...
    def monitor(self):
        ...
        while True:
            ...
            if now - timer_show_num_workers >= 0.1:
                timer_show_num_workers = time.time()
                request = simulation_pb2.Request(message_type=5, num_workers=self.num_workers + 1)
                response = self.stub.SendRequest(request)
    ...
    def show_parameters(self):
        while True:
            ...
            with threading.Lock():
                ...
                print(f"[Frontend] the number of workers = {min(self.num_workers + 1, len(self.workers))}")
            time.sleep(1)

    # 选择 worker
    def select_worker(self):  # 问题：num_workers 向上调整过大
        for i in range(min(self.num_workers + 1, len(self.workers))):
